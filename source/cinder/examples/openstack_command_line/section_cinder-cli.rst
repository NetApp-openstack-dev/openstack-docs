...
.. note::
   This command is only available in microversion 3.40. From Xena release, this
   command is performed using the storage with a safer and faster approach.
...
.. note::
   ...
   using the storage approach, becoming safer, faster and in most cases
   non-disruptive.
...
.. note::
   ...
   by ``netapp_migrate_volume_timeout`` driver option - it's default value is
   3600 as seconds. When the timeout is reached:

   - on disruptive operations the volume goes back as the original state,
     deleting the volume copy on the target path.

   - on non-disruptive operations, there is no way of stopping the migration,
     the volume status is set as ```maintenance``, the user must watch over the
     migration status and when it succeed, reset the volume state to the
     pre-migration status using:
