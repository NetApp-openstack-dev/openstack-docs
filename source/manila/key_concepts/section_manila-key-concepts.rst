...
evacuation or rebalancing.

As with share migration, a 2-phase approach is available, which allows
administrators control the right time to complete the operation, that ends on
...
":ref:`Table 6.10, “Manila API Overview - Share Server Migration<table-6.10>`".
...
- Source and destination clusters have the same ONTAP system version, which must
  be equal or greater then 9.4;
- Source and destination share network don't have different security services
  configurations;
