...
.. important::
    ...
    availability zone or to a fallback one. A share network can have multiple
...
created share network, a feature is available since Train release. It
...
Finally, it is possible to delete a share network subnet running the below
command.
...
``driver_handles_share_server=True`` mode. In the below example, a share
...
you are specifying an availability zone, you must make sure that the parent's
share share network contains a default share network subnet, or a share
