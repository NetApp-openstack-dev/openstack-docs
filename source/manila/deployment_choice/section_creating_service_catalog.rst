...
.. [#f3] ``netapp:adative_qos_policy_group`` capability expects that the
         ...
         higher and needs a cluster scoped account configured in driver options.

         This feature is only supported by ``driver_handles_share_servers`` set
         to ``False`` mode and will not work with ``Share Replication`` and
         ``Create Share From Snapshot Across Pools or Backends``.
